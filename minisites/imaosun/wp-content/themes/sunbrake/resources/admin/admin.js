...
/*!**************************************!*\
  ...
  \**************************************/
(function ($) {
  var $notice_container = $(".sunbrake-admin-big-notice--container");
  var selectedFrontPage = 0;

  if (!window.sunbrake_admin) {
    return;
  }

  var _window$sunbrake_admi = window.sunbrake_admin,
      builderStatusData = _window$sunbrake_admi.builderStatusData,
      getStartedData = _window$sunbrake_admi.getStartedData;
  ...
  function toggleProcessing(value) {
    $(window).on("beforeunload.sunbrake-admin-big-notice", function () {
      return true;
    });

    if (value) {
      $(".sunbrake-admin-big-notice").addClass("processing");
      $(".sunbrake-admin-big-notice .action-buttons").fadeOut();
    } else {
      $(".sunbrake-admin-big-notice").removeClass("processing");
    }
  }
  ...
  function installBuilder(callback) {
    ...
    $.get(builderStatusData.install_url).done(function () {
      ...
      $(window).off("beforeunload.sunbrake-admin-big-notice");
    });
  }
  ...
  $notice_root = $notice_container.closest(".sunbrake-admin-big-notice");
  $custom_close_button = $notice_root.find(".sunbrake-notice-dont-show-container");

  if ($custom_close_button.length) {
    $custom_close_button.on("click", function () {
      disableNotice();
      $notice_container.closest(".sunbrake-admin-big-notice").find("button.notice-dismiss").click();
    });
  } else {
  ...
  window.sunbrake_admin.showOverlay = showOverlay;
})(jQuery);
/******/ })()
;
