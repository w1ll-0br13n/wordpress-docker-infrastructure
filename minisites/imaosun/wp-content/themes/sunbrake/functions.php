...
function sunbrake_register_components( $components ) {
...
Hooks::prefixed_add_filter( 'components', 'sunbrake_register_components', 20 );
Theme::load(
	array(
 ...
 * @return Theme
 */
function sunbrake_theme() {
	return Theme::getInstance();
}

function sunbrake_assets() {
	return sunbrake_theme()->getAssetsManager();
}
...
sunbrake_theme()
	...
	->register_menus(
		array(
			'header-menu' => esc_html__( 'Header Menu', 'sunbrake' ),
			'footer-menu' => esc_html__( 'Footer Menu', 'sunbrake' ),
		)
	);
...
if ( ! apply_filters( 'kubio_is_enabled', false ) ) {
	sunbrake_assets()
		...
		->registerStylesheet( 'sunbrake-theme', '/theme/theme.css' );
	sunbrake_assets()->loadLocalGoogleFonts();
	//	foreach ( Defaults::get( 'fonts', array() ) as $family => $variants ) {
	//		sunbrake_assets()->addGoogleFont( $family, $variants );
	//	}

	add_action( 'wp_enqueue_scripts', 'sunbrake_print_color_scheme', 0 );
} else {
	sunbrake_assets()
		->registerStyle( 'sunbrake-theme', sunbrake_assets()->getBaseURL() . '/theme/fse-base-style.css' );
	Hooks::prefixed_add_filter( 'skip_google_fonts', '__return_true' );
}

function sunbrake_theme_add_woocommerce_support() {
	add_theme_support(
		'woocommerce',
		array(
			'product_grid' => array(
...
add_action( 'after_setup_theme', 'sunbrake_theme_add_woocommerce_support' );

add_filter( 'kubio/activation/after_activation_redirect_url', 'sunbrake_after_activation_redirect_url' );
function sunbrake_after_activation_redirect_url( $url ) {
...
function sunbrake_get_builder_plugin_slug() {
...
function sunbrake_theme_plugins( $plugins ) {

	$plugin_slug = sunbrake_get_builder_plugin_slug();

	$plugins[ $plugin_slug ] = array(
		'name'        => $plugin_slug === 'kubio-pro' ? __( 'Kubio PRO', 'sunbrake' ) : __( 'Kubio', 'sunbrake' ),
...
Hooks::prefixed_add_filter( 'theme_plugins', 'sunbrake_theme_plugins' );
...
Hooks::prefixed_add_action(
	...
	function ( $slug ) {
		...
		if ( $slug === sunbrake_get_builder_plugin_slug() ) {
                ...
                <div class="kubio-admin-row get-started-imported notice notice-success">
                    <div>
                        <p class="imported-title">
							...
							echo esc_html(
								sprintf(
									__( '%s design has been successfully imported!', 'sunbrake' ),
									sunbrake_theme()->getName()
								)
							);
							?>
                        </p>
                        <p class="imported-subtitle">
							...
							echo esc_html(
								sprintf(
									__( '%s design has been successfully imported! You can take a look at your new design or start editing it', 'sunbrake' ),
									sunbrake_theme()->getName()
								)
							);
							?>
                    ...
                    <div class="button imported-view-site-button">
                        <a href="<?php echo esc_url( site_url() ); ?>">
							<?php echo esc_html( __( 'View site', 'sunbrake' ) ); ?>
                        </a>
...
function sunbrake_try_online_url() {
	return 'https://kubiobuilder.com/go/try-theme/' . get_template();
}

add_filter( 'kubio/editor-try-online/url', 'sunbrake_try_online_url', 10 );

function sunbrake_render_header_style() {

	?>
    <base target="_top">
	<?php

	sunbrake_theme()->get( 'css' )->render();
}
add_action( 'wp_head', 'sunbrake_render_header_style', 100 );
