...
wp_localize_script(
	...
	array(
		...
		'builderStatusData' => array(
			'status'                          => sunbrake_theme()->getPluginsManager()->getPluginState( sunbrake_get_builder_plugin_slug() ),
			'install_url'                     => sunbrake_theme()->getPluginsManager()->getInstallLink( sunbrake_get_builder_plugin_slug() ),
			'activate_url'                    => sunbrake_theme()->getPluginsManager()->getActivationLink( sunbrake_get_builder_plugin_slug() ),
			'slug'                            => sunbrake_get_builder_plugin_slug(),
...
<div class="sunbrake-admin-big-notice--container">
	<div class="logo-holder">
        <h2><?php echo sprintf(__("Choose one of the %s predefined designs to start with:", 'sunbrake'), ucfirst(get_stylesheet())); ?></h2>
	</div>
	<div class="content-holder">
		<ul class="predefined-front-pages">
			<?php foreach ( \ColibriWP\Theme\Defaults::get( 'front_page_designs' ) as $sunbrake_design_index => $design ) : ?>
				<?php
					$sunbrake_design_selected = $sunbrake_design_index === 0 ? 'selected' : '';
				...
				<li data-index="<?php echo esc_attr( $design['index'] ); ?>"
					class="<?php echo $sunbrake_design_selected; ?>">
                    ...
                    <div class="predefined-front-page-card">
						...
						<div class="front-page-design-wrapper">
							<div class="design-preview-image"
								 style="background-image: url(<?php echo esc_attr( sunbrake_theme()->getAssetsManager()->getBaseURL() . "/images/{$preview_image_name}" ); ?>)"
							></div>
						</div>
